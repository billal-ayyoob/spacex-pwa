...
var CodeFileLoader = /** @class */ (function () {
    ...
    CodeFileLoader.prototype.getMergedOptions = function (options) {
        return tslib.__assign(tslib.__assign({}, this.config), options);
    };
    ...
    CodeFileLoader.prototype.handleSinglePath = function (location, options) {
        return tslib.__awaiter(this, void 0, void 0, function () {
            var normalizedFilePath, errors, content, sources, mergedDocument, mergedSource, e_2, loaded, source, e_3;
            return tslib.__generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 4:
                        sources = _a.sent();
                        if (sources.length) {
                            mergedDocument = flattenSources(sources, options);
                            mergedSource = {
                                document: mergedDocument,
                                location: location,
                            };
                            return [2 /*return*/, [mergedSource]];
                        }
                        return [3 /*break*/, 6];
                    case 5:
                        e_2 = _a.sent();
                        if (process.env['DEBUG']) {
                            console.error("Failed to load schema from code file \"" + normalizedFilePath + "\": " + e_2.message);
                        }
                        errors.push(e_2);
                        return [3 /*break*/, 6];
                    case 6:
                    ...
                    case 10:
                        loaded = _a.sent();
                        source = resolveSource(location, loaded, options);
                        if (source) {
    ...
    CodeFileLoader.prototype.handleSinglePathSync = function (location, options) {
        ...
        if (!options.noPluck) {
            try {
                ...
                var sources = graphqlTagPluck.gqlPluckFromCodeStringSync(normalizedFilePath, content, options.pluckConfig);
                if (sources.length) {
                    var mergedDocument = flattenSources(sources, options);
                    var mergedSource = {
                        document: mergedDocument,
                        location: location,
                    };
        ...
        if (!options.noRequire) {
            try {
                if (options && options.require) {
                    var cwdRequire = module$1.createRequire(options.cwd || process.cwd());
                    try {
                        for (var _b = tslib.__values(utils.asArray(options.require)), _c = _b.next(); !_c.done; _c = _b.next()) {
                            var m = _c.value;
                            cwdRequire(m);
                        }
                    }
                ...
                var source = resolveSource(location, loaded, options);
                if (source) {
                    return [source];
                }
            }
...
function flattenSources(sources, options) {
    var documents = sources.map(function (source) { return graphql.parse(source, options); });
    return graphql.concatAST(documents);
}
